...
const Share = ({ shareModal, setShareModal, podcast, cardId }) => {

    const closeShare = () => {
        setShareModal(false)
    }

    const targetPodcast = podcast.find(podcast => podcast.id == cardId)
            ...
            <main className='share__main'>
                <div className='share__wrapper'>
                    ...
                    <div className='share__box'>
                        <img className='share__art' src={targetPodcast ? targetPodcast.img : ''} alt='cover art' />
                        <h5 className='share__title'>{targetPodcast ? targetPodcast.podcast_title : ''}</h5>
                        <p className='share__descrip'>{targetPodcast ? targetPodcast.podcast_name : ''}</p>
                    </div>
